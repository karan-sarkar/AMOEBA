...
def train(train_loader, test_loader, model, crit, optimizer, epoch, print_freq):
    ...
    plt.hist([np.array(dlosses2),np.array(dlosses1)], 30, label=['Static ' + SOURCE_FLAG + ' Class Discrepancy', 'Static ' + TARGET_FLAG + ' Class Discrepancy'])
    plt.legend(loc='upper right')
    plt.savefig('Static-' + SOURCE_FLAG + TARGET_FLAG +  'class-discrep.pdf')
    plt.clf()
    
    plt.style.use('seaborn-deep')
    bins = np.linspace(0, 0.5, 30)
    
    print(SOURCE_FLAG, np.array(dlosses2).mean(), np.array(dlosses2).std())
    print(TARGET_FLAG, np.array(dlosses1).mean(), np.array(dlosses1).std())    


    plt.hist([np.array(blosses2), np.array(blosses1)], 30, label=['Static ' + SOURCE_FLAG + ' Box Discrepancy', 'Static '+ TARGET_FLAG +  ' Box Discrepancy'])
    plt.legend(loc='upper right')
    plt.savefig('Static-' + SOURCE_FLAG + TARGET_FLAG + 'box-discrep.pdf')
