...
def generate_save_names(cfg):
    print(f'{type(cfg.get("SAVE.MODEL_CORE_NAME")), cfg.get("SAVE.MODEL_CORE_NAME")}')

    print(f'{type(cfg.get("SAVE.MODEL_VERSION")), cfg.get("SAVE.MODEL_VERSION")}')
    ...
    if cfg.get("SAVE.DRIVE_BACKUP"):
        ...
        ### we will save it in our own path....
        CHECKPOINT_DIRECTORY = '/nethome/jbang36/j_amoeba/benchmark/odin/data/models' + MODEL_SAVE_FOLDER

    ### we need to modify the model_save_folder...
    MODEL_SAVE_FOLDER = os.path.join('/nethome/jbang36/j_amoeba/benchmark/odin/data/models', MODEL_SAVE_FOLDER)
