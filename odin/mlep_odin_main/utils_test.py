...
class TestUtils(unittest.TestCase):
    def test_std_flush(self):
        ...
        # No parameters.
        amoeba_utils.std_flush()
        self.assertEqual(red_stdout.getvalue(), "\n")
        # Parameters of different types.
        amoeba_utils.std_flush(42, "Hello", [1, 2, 3])
        self.assertEqual(red_stdout.getvalue(), "\n42 Hello [1, 2, 3]\n")
    ...
    def test_readable_time(self):
        ...
        self.assertTrue(datetime.datetime.strptime(amoeba_utils.readable_time(), "%M:%S").minute in
                        [datetime.datetime.now().minute, (datetime.datetime.now().minute - 1) % 60])
        self.assertTrue(datetime.datetime.strptime(amoeba_utils.readable_time(), "%M:%S").second in
                        [datetime.datetime.now().second, (datetime.datetime.now().second - 1) % 60])
        # Test format parameter.
        self.assertTrue(
            datetime.datetime.strptime(amoeba_utils.readable_time("%H:%M:%S"), "%H:%M:%S").hour in
            [datetime.datetime.now().hour, (datetime.datetime.now().hour - 1) % 24]
        )
        self.assertTrue(
            datetime.datetime.strptime(amoeba_utils.readable_time("%H:%M:%S"), "%H:%M:%S").minute in
            [datetime.datetime.now().minute, (datetime.datetime.now().minute - 1) % 60]
        )
        self.assertTrue(
            datetime.datetime.strptime(amoeba_utils.readable_time("%H:%M:%S"), "%H:%M:%S").second in
            [datetime.datetime.now().second, (datetime.datetime.now().second - 1) % 60]
        )

    def test_time_to_id(self):
        """Test if returned string is non-empty and contains appropriate alphabetic characters."""
        test_id = amoeba_utils.time_to_id()
