...
class BoxList:
    ...
    def clip(self, remove_empty=True):
        ...
        #### because we have things normalized, all the values should be between 0 and 1
    ...
    def clip_normalized(self, remove_empty = True):
        self.box.clamp_(min=0, max=1)
        epsilon = 0.20

        if remove_empty:
            box = self.box
            keep = (box[:, 3] > box[:, 1] + epsilon) & (box[:, 2] > box[:, 0] + epsilon)
