...
def evaluate_fixmatch(dataset, predictions):
    ...
    with tempfile.NamedTemporaryFile() as f:
        ...
        if type(dataset) == torch.utils.data.dataset.Subset:
            ...
            res = evaluate_predictions_on_coco(
                dataset.datasets[0].dataset.coco, coco_results['bbox'], path, 'bbox'
            )


        else: ## type of dataset is
            res = evaluate_predictions_on_coco(
...
def make_coco_detection(predictions, dataset):
    ...
    for id, pred in enumerate(predictions):
        ...
        ### we can't use this id2img because of concat dataset

        if len(pred) == 0:
            continue

        if type(dataset) == torch.utils.data.dataset.ConcatDataset:
            ### ConcatDataset -> Subset -> COCODataset
            if id >= len(dataset.datasets[0]):
                adjusted_id = id - len(dataset.datasets[0])
                orig_id = dataset.datasets[1].dataset.id2img[adjusted_id]
                img_meta = dataset.datasets[1].dataset.get_image_meta(id)
            else:
                adjusted_id = id
                orig_id = dataset.datasets[0].dataset.id2img[adjusted_id]
                img_meta = dataset.datasets[0].dataset.get_image_meta(id)

        elif type(dataset) == torch.utils.data.dataset.Subset:
            orig_id = dataset.dataset.id2img[id]
            img_meta = dataset.dataset.get_image_meta(id)
        else:
        ...
        if type(dataset) == torch.utils.data.dataset.ConcatDataset:
            if id >= len(dataset.datasets[0]): labels = [dataset.datasets[1].dataset.id2category[i] for i in labels]
            else: labels = [dataset.datasets[0].dataset.id2category[i] for i in labels]
        elif type(dataset) == torch.utils.data.dataset.Subset:
            labels = [dataset.dataset.id2category[i] for i in labels]
        else:
